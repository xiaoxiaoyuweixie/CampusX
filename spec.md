...
稳定规则 ID：`CHAT-UNREAD-001`。

- `chatService.getUnreadState` 接收空 `data`，返回 `{ hasUnread: boolean }`，沿用统一业务 envelope。仅使用云函数真实 `OPENID` 并校验账号存在且可用，不接受客户端指定查询身份。
- 从当前账号参与的全部 `active` 会话中查询本人角色的 `unreadCount > 0` 是否存在；不依赖会话列表前 50 条，不返回他人未读数或会话资料。发送累加与 `getMessages`/`markRead` 清零规则保持不变。
- `miniprogram/utils/unread.js` 统一控制原生 TabBar 第 4 项（索引 3）的红点。按用户确认的红点下移要求，使用 `setTabBarItem` 同时切换普通/选中消息图标：未读使用 `tab-message-unread.png` / `tab-message-active-unread.png`，已读或退出恢复原图；只用 `hideTabBarRedDot` 清理系统旧红点，不显示数字、不启用自定义 TabBar。查询失败、业务错误或返回结构无效均保留当前账号上次确认的状态。
- 登录后首次进入、每个 Tab 的 `onShow`、成功获取聊天消息后及 App 恢复前台立即请求刷新。前台登录期间，每次请求结束后间隔 5 秒再次查询；至多一个在途请求和一个合并补刷，不以固定定时器堆叠慢请求。消息页可见时订阅同一刷新结果更新列表，不另建轮询。
- App 进入后台暂停定时器并使在途结果失效，恢复前台重新查询。退出或换号清理旧状态，按账号和登录代次拒绝迟到结果；非 Tab 页暂时无法绘制原生红点时，在下一次 Tab 显示重绘。资料请求也不得在退出或换号后回写旧身份。
...
- 小程序端文案使用中文。
- 小程序保持清爽、轻量、校园感。
- 当前主色为 `#3B82F6`，辅助色为 `#60A5FA`，通用背景色为 `#F8FAFC`；首页、分类、发布、消息、聊天和我的页面及各自原生导航栏统一使用首页的浅蓝背景 `#EFF6FF`，原生 TabBar 同色。页面配置的窗口背景与页面底色保持一致，分类主容器/选中分类背景及聊天主容器同步；消息页的系统通知与私信列表（含空态、加载态区域）也使用 `#EFF6FF`。聊天页对方的文字消息气泡使用纯白底 `#FFFFFF` 和深灰文字 `#1F2937`，本人的文字消息气泡保留蓝底 `#3B82F6` 和白字，保留左右位置及各自圆角；图片消息、状态提示、其余卡片、输入区、操作区和弹窗保持各自既有表面样式。首页细节见 `HOME-UI-001`。
...
- 首页以 Figma 文件 `ypmVzm832Ho7LpKxfyzsnO` 的“首页最新UI”分区 `514:2332` 为视觉来源：首页画板 `243:1862`、底部导航组件板 `501:266`。来源、导出资产和验收情况记录在 `docs/design/2026-09-22-home-ui/`，不使用“过程草稿”作为实现依据。
- 首页页面、原生导航栏及全局原生 TabBar 底色为 `#EFF6FF`；保留真实微信状态栏和胶囊，不复刻稿件中的系统状态栏。首页采用学校与搜索栏、校园横幅、五类入口和双列推荐资源布局。
- 首页商品卡片按用户最新确认统一采用浅蓝底 `#EFF6FF`，通过 `product-card` 的 `variant="home"` 使用首页样式；标题保留两行，校区和卖家/浏览辅助文字采用 20rpx 以容纳真实内容。商品封面、标题、价格、校区、卖家、浏览数及推荐顺序继续来自现有接口，不将 Figma 示例商品写入小程序或数据库。其他页面沿用组件的默认样式。
- 分类图标按稳定分类 ID 映射至 Figma 资产，保留后台返回的分类名称与顺序；未知分类沿用原图标降级。学校继续显示既有静态“西南大学”，横幅只作展示，不增加定位、切校或跳转业务。
- 分类页侧栏底色同为 `#EFF6FF`，五类入口使用用户于 2026-09-23 提供的透明主体素材，来源记录在 `docs/design/2026-09-23-category-icons-transparent/`，由 `miniprogram/utils/category-presentation.js` 的分类页选项映射至 `miniprogram/images/category/`；首页继续使用原 Figma 资产。侧栏图标为 64rpx，保留选中项的蓝色文字和左侧标记，以及原分类筛选、排序和商品跳转行为。
- 发布页操作栏位于商品描述卡片之后，参与页面正常布局并随页面滚动，不固定在 TabBar 上方；使用透明背景，不显示白色填充和顶部分隔线，不再为固定操作栏预留大块底部空间。蓝色发布按钮及提交行为保持不变。
- 保留搜索确认/取消、分类及商品跳转、整页滚动、下拉刷新、AI 入口拖动和对话功能。页面路径、推荐算法及云函数接口保持现有契约。
- AI 助手输入栏不显示独立发送按钮，输入框延伸至原按钮右侧边缘，保留“问问校园资源相关问题”占位文案；通过手机键盘的发送键提交，继续拒绝空白内容和回复期间的重复提交，并显示对话中的“正在回复...”状态。
- 五个 Tab 的普通/选中图标均使用最新导航组件板导出资产。消息未读变体基于新消息图标叠加既有下移红点，保持原文件路径及 `CHAT-UNREAD-001` 的权威未读状态、轮询和账号隔离逻辑；不改为自定义 TabBar。
- WXML/WXSS 编译和逻辑回归不代表真机视觉验收完成；iOS/Android 仍需验证搜索键盘、不同屏宽、整页滚动、AI 浮层及未读图标。
...
稳定规则 ID：`CHAT-CONTACT-DIALOG-001`。

- 交换微信继续使用原生 `page-container position="center"`，通过容器内的 flex 布局让弹窗在页面可视区域水平、垂直居中，不用固定顶部距离适配机型。
- 弹窗宽高受父容器约束，小屏或键盘导致空间不足时允许内容滚动；打开前尝试收起已打开的键盘，保留文字输入和已存草稿。键盘关闭时的居中不能依赖这一步。
- 外层空白区域将点击交给原生遮罩，弹窗内部可交互；取消、遮罩、原生返回均沿用既有关闭处理。提醒文案、脱敏、复制鉴权及电话行为保持不变。
- 布局与逻辑验证不能代替 iOS/Android 真机验收；不同屏幕尺寸、键盘开关和原生返回须在体验版复验。
...
- `admin-web/.env` 是本地配置文件，不作为共享配置来源；与 `project.private.config.json` 一起从 Git 跟踪中排除，本机文件保留，新电脑单独恢复配置。
